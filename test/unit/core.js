...
QUnit.test( "selector state", function( assert ) {
	expect( 18 );

	var test;

	test = jQuery( undefined );
	assert.equal( test.selector, "", "Empty jQuery Selector" );
	assert.equal( test.context, undefined, "Empty jQuery Context" );

	test = jQuery( document );
	assert.equal( test.selector, "", "Document Selector" );
	assert.equal( test.context, document, "Document Context" );

	test = jQuery( document.body );
	assert.equal( test.selector, "", "Body Selector" );
	assert.equal( test.context, document.body, "Body Context" );

	test = jQuery("#qunit-fixture");
	assert.equal( test.selector, "#qunit-fixture", "#qunit-fixture Selector" );
	assert.equal( test.context, document, "#qunit-fixture Context" );

	test = jQuery("#notfoundnono");
	assert.equal( test.selector, "#notfoundnono", "#notfoundnono Selector" );
	assert.equal( test.context, document, "#notfoundnono Context" );
	...
	assert.equal( test.selector, "#qunit-fixture", "#qunit-fixture Selector" );
	assert.equal( test.context, document.body, "#qunit-fixture Context" );
	...
	assert.equal( test.selector, "#qunit-fixture", "#qunit-fixture find Selector" );
	assert.equal( test.context, document.body, "#qunit-fixture find Context" );
});
...
QUnit.test( "isNumeric", function( assert ) {
	...
	assert.ok( t(answer), "Custom .toString returning number");
	...
	assert.equal( t(rong), false, "Custom .toString returning non-number");
...
QUnit.test("jQuery.each(Object,Function)", function( assert ) {
	...
	assert.equal( i, 2, "Iteration over document.styleSheets" );
});
...
QUnit.test("jQuery.proxy", function( assert ) {
	...
	var test2, test3, test4, fn, cb,
		test = function(){ assert.equal( this, thisObject, "Make sure that scope is set properly." ); },
	...
	// Partial application
	test2 = function( a ){ assert.equal( a, "pre-applied", "Ensure arguments can be pre-applied." ); };
	jQuery.proxy( test2, null, "pre-applied" )();

	// Partial application w/ normal arguments
	test3 = function( a, b ){ assert.equal( b, "normal", "Ensure arguments can be pre-applied and passed as usual." ); };
	jQuery.proxy( test3, null, "pre-applied" )( "normal" );

	// Test old syntax
	test4 = { "meth": function( a ){ assert.equal( a, "boom", "Ensure old syntax works." ); } };
...
QUnit.test("jQuery.parseXML", 8, function( assert ) {
	var xml, tmp;
	try {
...
if ( !/iphone os 7_/i.test( navigator.userAgent ) ) {
	testIframeWithCallback( "document ready when jQuery loaded asynchronously (#13655)", "core/dynamic_ready.html", function( ready ) {
		expect( 1 );
		QUnit.assert.equal( true, ready, "document ready correctly fired when jQuery is loaded after DOMContentLoaded" );
	});
}
...
testIframeWithCallback( "Tolerating alias-masked DOM properties (#14074)", "core/aliased.html",
	function( errors ) {
		expect( 1 );
		QUnit.assert.deepEqual( errors, [], "jQuery loaded" );
	}
);
...
testIframeWithCallback( "Don't call window.onready (#14802)", "core/onready.html",
	function( error ) {
		expect( 1 );
		QUnit.assert.equal( error, false, "no call to user-defined onready" );
	}
);
