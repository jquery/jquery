...
QUnit.test( "jQuery.Deferred.then - filtering (fail)", function( assert ) {
	...
	jQuery.Deferred().resolve().then( null, () => {
...
QUnit.test( "[PIPE ONLY] jQuery.Deferred.pipe - filtering (fail)", function( assert ) {
	...
	jQuery.Deferred().resolve().pipe( null, () => {
		assert.ok( false, "then should not be called on resolve" );
	} ).then( done.pop() );
...
QUnit.test( "jQuery.Deferred.then - filtering (progress)", function( assert ) {
	...
	defer.notify( 2, 3 ).then( null, null, () => {
...
QUnit.test( "jQuery.Deferred.then - deferred (progress)", function( assert ) {
	...
	piped.progress( function( result ) {
		return jQuery.Deferred().resolve().then( () => {
			return result;
...
QUnit.test( "jQuery.Deferred.then - context", function( assert ) {
	...
	let defer, piped, defer2, piped2,
		context = { custom: true },
		done = jQuery.map( new Array( 5 ), () => {
			return assert.async();
		} );
...
QUnit.test( "jQuery.Deferred.then - progress and thenables", function( assert ) {
	...
	let trigger = jQuery.Deferred().notify(),
		expectedProgress = [ "baz", "baz" ],
		done = jQuery.map( new Array( 2 ), () => {
			return assert.async();
		} ),
		failer = function( evt ) {
			return () => {
	...
	trigger.then( null, null, () => {
		let notifier = jQuery.Deferred().notify( "foo" );
		setTimeout( () => {
...
QUnit.test( "jQuery.Deferred - notify and resolve", function( assert ) {
	...
	let notifiedResolved = jQuery.Deferred().notify( "foo" )/*xxx .resolve( "bar" )*/,
		done = jQuery.map( new Array( 3 ), () => {
			return assert.async();
		} );
	...
	notifiedResolved.pipe( null, null, () => {
		return "baz";
	} ).progress( function( v ) {
	...
	notifiedResolved.then( null, null, () => {
		return "baz";
	} ).progress( function( v ) {
...
QUnit.test( "jQuery.when(nonThenable) - like Promise.resolve", function( assert ) {
	...
	let defaultContext = ( function getDefaultContext() {
		return this;
	} )(),
	...
	jQuery.each( {
		...
		let code = "jQuery.when( " + message + " )",
			onFulfilled = function( method ) {
				let call = code + "." + method;
...
QUnit.test( "jQuery.when - always returns a new promise", function( assert ) {
	...
	jQuery.each( {
		...
		let result = jQuery.when.apply( jQuery, args );
...
QUnit.test( "jQuery.when - notify does not affect resolved", function( assert ) {
	...
	let a = jQuery.Deferred().notify( 1 ).resolve( 4 ),
