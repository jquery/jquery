...
QUnit.test("on(), namespaced events, cloned events", 18, function( assert ) {
...
QUnit.test("off(eventObject)", function() {
	...
	function assert( expected ){
		...
		QUnit.assert.equal( num, expected, "Check the right handlers are triggered" );
	}
...
if ( window.onbeforeunload === null &&
	...
	QUnit.asyncTest("on(beforeunload)", 1, function( assert ) {
...
QUnit.test(".on()/.off()", function( assert ) {
	...
	jQuery("#body").on("click", "#foo", true, function( e ){ assert.equal( e.data, true, "on with event data" ); });
	jQuery("#foo").trigger("click");
	jQuery("#body").off("click", "#foo");

	// Test binding with trigger data
	jQuery("#body").on("click", "#foo", function(e, data){ assert.equal( data, true, "on with trigger data" ); });
	jQuery("#foo").trigger("click", true);
	jQuery("#body").off("click", "#foo");

	// Test binding with different this object
	jQuery("#body").on("click", "#foo", jQuery.proxy(function(){ assert.equal( this["foo"], "bar", "on with event scope" ); }, { "foo": "bar" }));
	jQuery("#foo").trigger("click");
	jQuery("#body").off("click", "#foo");
...
QUnit.test( "make sure events cloned correctly", 18, function( assert ) {
	var clone,
		fixture = jQuery("#qunit-fixture"),
...
if ( !(/firefox/i.test( window.navigator.userAgent )) ) {
	QUnit.test( "Check order of focusin/focusout events", 2, function( assert ) {
		var focus, blur,
			input = jQuery( "#name" );
