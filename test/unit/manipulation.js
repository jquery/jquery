...
function testText( assert, valueObj ) {
...
QUnit.test( "text(String)", function( assert ) {
	testText( assert, manipulationBareObj );
});

QUnit.test( "text(Function)", function( assert ) {
	testText( assert, manipulationFunctionReturningObj );
});
...
function testAppendForObject( assert, valueObj, isFragment ) {
	var $base,
...
function testAppend( assert, valueObj ) {

	expect( 78 );

	testAppendForObject( assert, valueObj, false );
	testAppendForObject( assert, valueObj, true );
...
QUnit.test( "append(String|Element|Array<Element>|jQuery)", function( assert ) {
	testAppend( assert, manipulationBareObj );
});

QUnit.test( "append(Function)", function( assert ) {
	testAppend( assert, manipulationFunctionReturningObj );
});
...
function testReplaceWith( assert, val ) {
...
QUnit.test( "replaceWith(String|Element|Array<Element>|jQuery)", function( assert ) {
	testReplaceWith( assert, manipulationBareObj );
});

QUnit.test( "replaceWith(Function)", function( assert ) {
	expect( testReplaceWith( assert, manipulationFunctionReturningObj ) + 1 );
...
QUnit.test( "Empty replaceWith (#13401; #13596)", 8, function( assert ) {
	var $el = jQuery( "<div/>" ),
		tests = {
			"empty string": "",
			"empty array": [],
...
function testHtml( assert, valueObj ) {
	expect( 40 );
...
QUnit.test( "html(String|Number)", function( assert ) {
	testHtml( assert, manipulationBareObj );
});

QUnit.test( "html(Function)", function( assert ) {
	testHtml( assert, manipulationFunctionReturningObj );
});
...
QUnit.test( "remove() event cleaning ", 1, function( assert ) {
...
QUnit.test( "remove() in document order #13779", 1, function( assert ) {
	var last,
...
QUnit.test( "detach() event cleaning ", 1, function( assert ) {
	var count, first, cleanUp;
...
QUnit.test( "jQuery.buildFragment - no plain-text caching (Bug #6779)", function( assert ) {
...
QUnit.test( "jQuery.buildFragment - plain objects are not a document #8950", function( assert ) {

	expect( 1 );

	try {
		jQuery( "<input type='hidden'>", {} );
		assert.ok( true, "Does not allow attribute object to be treated like a doc object" );
	} catch ( e ) {}
});
...
QUnit.test( "Index for function argument should be received (#13094)", 2, function( assert ) {
	var i = 0;
...
QUnit.test( "Make sure jQuery.fn.remove can work on elements in documentFragment", 1, function( assert ) {
...
QUnit.test( "Make sure specific elements with content created correctly (#13232)", 20, function( assert ) {
	var results = [],
		args = [],
		elems = {
...
QUnit.test( "Validate creation of multiple quantities of certain elements (#13818)", 44, function( assert ) {
