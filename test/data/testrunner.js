...
(function() {
	...
	QUnit.config.urlConfig.push( {
		id: 'jqdata',
		label: 'Check jQuery.data',
		tooltip: 'Trigger "QUnit.expectJqData" for all tests instead of just the ones that call it'
	});
	...
	this.moduleTeardown = function() {
		...
		if ( this.checkJqData || QUnit.urlParams.jqdata ) {
			for ( i in jQuery.cache ) {
				expectedKeys = expectedDataKeys[i];
				actualKeys = jQuery.cache[i] ? keys( jQuery.cache[i] ) : jQuery.cache[i];
				if ( !QUnit.equiv( expectedKeys, actualKeys ) ) {
					deepEqual( expectedKeys, actualKeys, 'Expected keys exist in jQuery.cache' );
				}
			...
			for ( i in expectedDataKeys ) {
				deepEqual( expectedDataKeys[i], undefined, 'No unexpected keys were left in jQuery.cache' );
				delete expectedDataKeys[i];
			}
		}
