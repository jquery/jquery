 ...
 * Works with versions 4.0.0-beta.1 and later
 */
...
async function buildRelease( { version } ) {
	...
	await exec( "npm install", { cwd: releaseFolder } );

	// Build the release
	console.log( `Building jQuery ${ version }...` );
	await exec( "npm run build:all", {
		cwd: releaseFolder,
		env: {
			VERSION: version
		}
	...
	await exec( "npm pack", { cwd: releaseFolder } ).then( ( { stdout } ) =>
		console.log( stdout )
	);
