...
jQuery.event = {
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		event.namespaceRe = event.namespace ?
	...
	dispatch: function( event ) {
		...
		while ( (matched = handlerQueue[ i++ ]) && !event.isPropagationStopped() ) {
			...
			while (
				...
				// 2) have namespace(s) a subset or
				//    equal to those in the bound event (both can have no namespace).
				if ( !event.namespaceRe || event.namespaceRe.test( handleObj.namespace ) ) {
	...
	props: "altKey bubbles cancelable" +
		"ctrlKey currentTarget eventPhase" +
		"metaKey relatedTarget shiftKey" +
