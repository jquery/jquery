module.exports = (grunt) ->
	...
	readOptionalJSON = (filepath) ->
		data = {}
		try data = grunt.file.readJSON filepath
		data
	fs = require "fs"
	...
	grunt.initConfig
		...
		compare_size:
			...
			options:
				compress:
					gz: (contents) -> gzip.zip( contents, {} ).length
	...
	grunt.registerTask "testswarm", (commit, configFile) ->
		jobName = undefined
		...
		tests.forEach (test) -> runs[test] = config.testUrl + commit + "/test/index.html?module=" + test
	...
	grunt.registerTask "selector", "Build Sizzle-based selector module", ->
		cfg = grunt.config "selector"
		name = cfg.destFile
		sizzle =
			api: grunt.file.read cfg.apiFile
			src: grunt.file.read cfg.srcFile
		compiled = parts = undefined
	...
	grunt.registerMultiTask "build", "Concatenate source (include/exclude modules with +/- flags), embed date/version", ->

			# Concat specified files.
			compiled = ""
			modules = @flags
			optIn = !modules["*"]
			explicit = optIn or Object.keys(modules).length > 1
			name = @data.dest
			src = @data.src
			deps = {}
			excluded = {}
			...
			excluder = (flag, needsFlag) ->
				...
				if optIn? and !modules[ flag ] and !modules[ "+" + flag ]
					excluded[ flag ] = false

				# explicit or inherited strong exclusion
				if excluded[ needsFlag ] or modules[ "-" + flag ]
					excluded[ flag ] = true

				# explicit inclusion overrides weak exclusion
				else if !excluded[ needsFlag ] and ( modules[flag] or modules[ "+" + flag ] )

					delete excluded[ needsFlag ]
					# ...all the way down
					if deps[ needsFlag ]
						deps[ needsFlag ].forEach (subDep) ->
							modules[ needsFlag ] = true
							excluder needsFlag, subDep

			# append commit id to version
			version += " #{process.env.COMMIT}" if process.env.COMMIT
			...
			src.forEach (filepath) ->
				...
				if filepath.needs?
					...
					filepath.needs.forEach (needsFlag) -> excluder flag, needsFlag
			...
			src.forEach (filepath) ->
				...
				if flag?
					...
					if excluded[ flag ] == undefined
						...
						# output list
						specified = true if modules[ "+#{flag}" ]?
					...
					# by the user
					if explicit? and specified?
						messages.forEach (message) -> grunt.log.writetableln [ 27, 30 ], message

				compiled += grunt.file.read( filepath ) unless omit?
	...
	grunt.registerTask "dist", ->
		stored = flags = paths = nonascii = undefined
		...
		paths = [].concat( stored, flags ).filter (path) -> path != "*"
	...
	grunt.registerMultiTask "pre-uglify", ->
		banner = @options().banner

		@files.forEach (mapping) ->
			# Join src
			input = mapping.src.map( (file) ->
			...
			grunt.file.write mapping.dest, ( banner or "" ) + input
	...
	grunt.registerMultiTask "post-uglify", ->
		@files.forEach (mapping) ->
			mapFileName = mapping.src[0]
		...
		@options().tempFiles.forEach (fileName) -> fs.unlink fileName
